  ...
  .provider('$dropdown', function() {
    ...
    this.$get = ["$window", "$rootScope", "$tooltip", "$timeout", function($window, $rootScope, $tooltip, $timeout) {
      ...
      function DropdownFactory(element, config) {
		...
		var pEl = typeof evt.target.closest === "function" && evt.target.closest('.customScrollbar');
          if(evt.target === element[0] && pEl ) return;
          return evt.target !== element[0] && !pEl && $dropdown.hide();
        }
    ...
    this.$get = ["$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$sce", "dimensions", "$$rAF", "$timeout", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $sce, dimensions, $$rAF, $timeout) {
      ...
      function TooltipFactory(element, config) {
        ...
        $tooltip.show = function() {
			...
			var attrbind = element.attr('bs-enabled');
			attrbind = (typeof attrbind === 'undefined' || attrbind == 'true' || attrbind == '1') ? true : false;

          if (!options.bsEnabled || $tooltip.$isShown || !attrbind) return;
